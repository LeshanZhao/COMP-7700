...
public class WebDataAdapter implements WebDataAccess
{
    ...
    private static String path = WebDataAdapter.class.getResource("").getPath();
    ...
    @Override
    public int connect(){
        return connect(this.database_url);
    }
    @Override
    ...
    public ArrayList<ProductModel> cast_Products_ToProductModels(ArrayList<Product> found_result_Products){
        ArrayList<ProductModel> found_result_ProductModels = new ArrayList<ProductModel>();
        for (Product p: found_result_Products){
            ProductModel pm = new ProductModel();
            pm.setProductID(p.getProductID());
            pm.setProductName(p.getProductName());
            pm.setProductDescription(p.getProductDescription());
            pm.setQuantityOnHand(p.getQuantityOnHand());
            pm.setPrice(p.getPrice());
            pm.setDateAdded(p.getDateAdded());
            pm.setActive(p.getActive());
            pm.setDateModified(p.getDateModified());
            pm.setSupplierID(p.getSupplierID());
//            pm.setProductDetailsList(p.getProductDetailsList());
            pm.setProductSupplier(p.getProductSupplier());

            found_result_ProductModels.add(pm);
        }

        return found_result_ProductModels;
    }


    private BufferedImage getDetailImage_FromProductDetailList(ArrayList<ProductDetails> productDetailList){
        BufferedImage detailImg = null;

        for (ProductDetails productDetails: productDetailList)
        {
            if(productDetails.getDetailsName().equals("Image"))
            {
                System.out.println(productDetails.getDetailsValue());
                detailImg = ImportImageFromPath(productDetails.getDetailsValue());
            }
        }
        if (detailImg == null){
            System.out.println("Failed fetching detailed image from system: may not exist");
        }
        return detailImg;
    }


    public void saveImages_FromProduct_ToProductImgModel(ArrayList<Product> found_result_Products,
                                                          ArrayList<ProductImageModel> found_ProductImages){

        // Cast Buffered Image/images (image and detail image) to Bytes
        for (int i=0; i < found_result_Products.size(); i++) {
            ProductImageModel nxt_ProductImgModel = new ProductImageModel();

            byte[] product_img = null;
            String img_type = "png";
            BufferedImage buffered_Img = found_result_Products.get(i).getProductImage();
            try {
                ByteArrayOutputStream img_ByteStream = new ByteArrayOutputStream();
                ImageIO.write(buffered_Img, img_type, img_ByteStream);

                product_img = img_ByteStream.toByteArray();
                System.out.println( "An Image is casted to ByteArray");
            } catch (IOException e1){
                e1.printStackTrace();
                System.out.println( "Failed to write the image to ByteArrayOutputStream.");
            }
            nxt_ProductImgModel.set_product_Image(product_img);



            byte[] product_Detail_img = null;
            String detail_img_type = "png";
            BufferedImage buffered_Detail_Img = null;
            buffered_Detail_Img = getDetailImage_FromProductDetailList(found_result_Products.get(i).getProductDetailsList());     // no detailed image? Should be detailed

            try {
                ByteArrayOutputStream detail_img_ByteStream = new ByteArrayOutputStream();
                ImageIO.write(buffered_Detail_Img, detail_img_type, detail_img_ByteStream);

                product_Detail_img = detail_img_ByteStream.toByteArray();
                System.out.println( "An Detail Image is casted to ByteArray");
            } catch (IOException e1){
                e1.printStackTrace();
                System.out.println( "Failed to write the detail image to ByteArrayOutputStream.");
            }
            nxt_ProductImgModel.set_product_Detail_Image(product_Detail_img);


            found_ProductImages.add(nxt_ProductImgModel);
